...
# Check if changelogs exist and update the version in the package
...
prepare-release:
	...
	if grep -qE "^## \[$$new_version\]" CHANGELOG.md; then \
		...
		echo "$(TEXT_COLOR_RED)$(TEXT_BOLD)Pre-release$(TEXT_RESET) version detected!" || true; \
	else \
